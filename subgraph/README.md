...
| entity | what |
|---|---|
| `Plot` | the address — which is the place — its owner, salt, when and by whom it was claimed, the current `note`, the `implementation` it is pointed at, whether it is `sealed`, its `name` under groundstate.eth, and the address read as coordinates: `x`, `y` (80 bits each), `cell` (first 13 hex digits, the metre of ground in the walkable world) and `tile` (first 9, the 256-metre tile the world files things by) |
